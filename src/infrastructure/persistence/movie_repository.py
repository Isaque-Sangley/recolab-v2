...
class MovieRepository(IMovieRepository):
    ...
    async def find_all(self, limit: int = 100, offset: int = 0) -> List[Movie]:
        ...
        stmt = select(MovieModel).order_by(MovieModel.title).limit(limit).offset(offset)
        result = await self.session.execute(stmt)
    ...
    async def exists(self, entity_id: MovieId) -> bool:
        ...
        stmt = select(func.count()).select_from(MovieModel).where(MovieModel.id == int(entity_id))
        result = await self.session.execute(stmt)
    ...
    async def find_well_rated(
        self, min_avg_rating: float = 4.0, min_rating_count: int = 10, limit: int = 100
    ) -> List[Movie]:
    ...
    async def find_by_year_range(
        self, start_year: int, end_year: int, limit: int = 100
    ) -> List[Movie]:
        """Busca filmes por faixa de ano"""
        stmt = (
            select(MovieModel)
            .where(and_(MovieModel.year >= start_year, MovieModel.year <= end_year))
    ...
    async def get_movie_stats(self) -> dict:
        ...
        popular_stmt = select(MovieModel).order_by(MovieModel.rating_count.desc()).limit(10)
