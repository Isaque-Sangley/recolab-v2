...
from ...domain.events import ModelType, ModelStatus
...
class ModelRepository(IModelRepository):
    ...
    async def exists(self, entity_id: str) -> bool:
        ...
        stmt = select(func.count()).select_from(ModelMetadataModel).where(
            ModelMetadataModel.id == entity_id
        )
        result = await self.session.execute(stmt)
    ...
    async def compare_versions(
        ...
        for metric_name in all_metrics:
            ...
            comparison["metrics"][metric_name] = {
                ...
                "winner": "version_b" if value_b > value_a else "version_a" if value_a > value_b else "tie"
            }
    ...
    async def get_model_stats(self) -> dict:
        ...
        for model_type in ModelType:
            type_stmt = select(func.count()).select_from(ModelMetadataModel).where(
                ModelMetadataModel.model_type == model_type.value
            )
