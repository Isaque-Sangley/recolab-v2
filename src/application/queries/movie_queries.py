...
from ..dtos import MovieDTO, MovieDetailDTO, SearchMoviesRequest, FilterMoviesRequest
...
class GetMovieDetailsQuery:
    ...
    async def execute(self, movie_id: int) -> Optional[MovieDetailDTO]:
        ...
        similar_movies = []
        if movie.genres:
            similar = await self.movie_repository.find_by_genres(
                movie.genres,
                limit=10
            )
...
class FilterMoviesQuery:
    ...
    async def execute(self, request: FilterMoviesRequest) -> List[MovieDTO]:
        ...
        if request.genres:
            movies = await self.movie_repository.find_by_genres(
                request.genres,
                limit=request.limit
            )
